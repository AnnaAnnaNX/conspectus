...
import { fragmentFromString, selectDivs } from "@/tests/selection/selection";
import {checkExistenceElement, checkExistenceSpanElement, removeEmptyLines} from "./utils";
...
describe("selectDiv", () => {
  it("III", async () => {
    ...
    assert.equal(removeEmptyLines(realOutputHTML as string), removeEmptyLines(outputHTML));
  });
...
`;
  ...
  it("III - some divs in middle", async () => {
    ...
    const realOutputHTML: string = fragment.querySelector(".content")?.outerHTML || '';
    assert.equal(removeEmptyLines(realOutputHTML.toString()), removeEmptyLines(outputHTML.toString()));
  });
});
